...
const App = () => {
  ...
  const fetchJson = useCallback(
    async (path, params = {}) => {
      ...
      if (!response.ok) throw new Error(`Content request failed: ${path}`)
  ...
  const fetchHomeSections = useCallback(async () => {
    if (!apiKey) {
      setErrorMessage('API key is missing. Add VITE_TMDB_API_KEY in frontend/.env')
      return
    }
    ...
    try {
      ...
      console.error(error)
      setErrorMessage('Unable to load movie content right now. Please try again soon.')
    } finally {
      setLoading(false)
    }
  }, [apiKey, fetchJson])
      ...
      <div className="wrapper">
        <header className="hero" style={heroBackdrop ? { backgroundImage: `url(${heroBackdrop})` } : undefined}>
          ...
          <div className="hero-content">
            ...
            <p>
              Explore rich movie data with powerful filters, detailed insights, and high-quality trailer playback in
