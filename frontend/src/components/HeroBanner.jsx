...
const HeroBanner = ({ movie, genresMap, onPlayTrailer, onMoreInfo, onToggleMyList, isInMyList }) => {
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-r from-[#16072f]/90 via-[#090312]/70 to-black/80" />
      ...
      <div className="relative z-10 flex h-full items-end px-4 pb-24 md:px-12">
        <div className="max-w-2xl space-y-4">
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            <button onClick={() => onPlayTrailer(movie)} className="rounded-xl bg-white px-6 py-2 font-semibold text-black hover:bg-white/90">▶ Play Trailer</button>
            <button onClick={() => onToggleMyList(movie)} className="rounded-xl border border-white/30 bg-white/10 px-6 py-2 font-semibold text-white hover:bg-white/20">{isInMyList ? '✓ In My List' : '➕ Add to My List'}</button>
            <button onClick={() => onMoreInfo(movie)} className="rounded-xl border border-white/30 bg-white/10 px-6 py-2 font-semibold text-white hover:bg-white/20">ℹ More Info</button>
          </div>
        </div>
      </div>
    </section>
