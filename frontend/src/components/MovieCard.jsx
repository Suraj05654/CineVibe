...
export default function MovieCard({ movie }) {
  ...
  return (
    <div className="movie-card flex flex-col items-center w-full h-full">
      <img src={getPosterUrl(movie.poster_path)} alt={movie.title} className="w-full aspect-[2/3] object-cover mb-4" />
      ...
      <div className="w-full flex flex-col flex-1">
        ...
        {loading ? (
          <button disabled className="mt-auto w-full bg-gradient-to-r from-[#D6C7FF] to-[#AB8BFF] text-primary font-bold py-2 rounded-lg opacity-70 cursor-not-allowed">
            Loading...
          </button>
        ) : trailerUrl ? (
          <>
            <button
              ...
              className="mt-auto w-full bg-gradient-to-r from-[#D6C7FF] to-[#AB8BFF] text-primary font-bold py-2 rounded-lg hover:opacity-90 transition-opacity shadow-md"
            >
              Play Trailer
            </button>
            {showModal && <TrailerModal trailerUrl={trailerUrl} onClose={() => setShowModal(false)} />}
          </>
        ) : (
          <button disabled className="mt-auto w-full bg-gradient-to-r from-[#D6C7FF] to-[#AB8BFF] text-primary font-bold py-2 rounded-lg opacity-70 cursor-not-allowed">
