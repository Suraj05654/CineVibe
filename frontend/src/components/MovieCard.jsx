...
const MovieCard = ({ movie, onPlayTrailer, onMoreInfo, onToggleMyList, isInMyList, onRequireLogin, user }) => {
    ...
    <article className="group relative overflow-visible">
      ...
      <div className="mt-2 flex items-center justify-between gap-2 px-1">
        ...
        <div className="flex gap-1 text-white/85">
          <button onClick={() => onPlayTrailer(movie)} className="rounded-xl bg-white/10 px-1.5 py-0.5 text-xs hover:bg-white/20">▶</button>
          <button onClick={handleToggleList} className="rounded-xl bg-white/10 px-1.5 py-0.5 text-xs hover:bg-white/20">{isInMyList ? '✓' : '➕'}</button>
        </div>
      </div>
    </article>
  );
